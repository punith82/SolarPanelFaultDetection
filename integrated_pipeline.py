...
DEFAULT_YOLO_MODEL = ROOT_DIR / 'runs' / 'detect' / 'runs' / 'panel_detection' / 'yolov8n_panels' / 'weights' / 'best.pt'
DEFAULT_OUTPUT_DIR = ROOT_DIR / 'runs' / 'panel_detection' / 'fault_reports'
...
def parse_args():
    ...
    parser.add_argument('--yolo', type=str, default=str(DEFAULT_YOLO_MODEL),
                        help='Path to YOLOv8 weights file')
    parser.add_argument('--eff_model', type=str, default='best', choices=['best', 'final'],
...
def build_panel_list(boxes, image_shape):
    ...
    for box in boxes:
        ...
        x1, y1, x2, y2 = [int(max(0, round(v))) for v in coords]
        xc = (x1 + x2) / 2
        yc = (y1 + y2) / 2
        panels.append({
            'box': box,
...
def annotate_image(image: np.ndarray, detections: list, output_path: Path):
    annotated = image.copy()
    for detection in detections:
        x1, y1, x2, y2 = detection['x1'], detection['y1'], detection['x2'], detection['y2']
        label = f"{detection['panel_id']}:{detection['predicted_class']} ({detection['confidence']*100:.1f}%)"
        color = (0, 200, 0) if detection['predicted_class'] == 'normal' else (0, 120, 255)
        cv2.rectangle(annotated, (x1, y1), (x2, y2), color, 2)
        text_y = max(y1 - 8, 12)
        cv2.putText(annotated, label, (x1, text_y), cv2.FONT_HERSHEY_SIMPLEX,
                    0.45, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.imwrite(str(output_path), annotated)
    print(f'[INFO] Annotated image saved to: {output_path}')


def process_top_view_image(image_path: Path, yolo_model, eff_model, output_dir: Path,
                           save_crops: bool = False, crop_dir: Path = None,
    ...
    # Clear old crops before processing new image
    ...
    results = yolo_model.predict(source=str(image_path), conf=conf_threshold,
    ...
    for panel in panels:
        ...
        if crop.size == 0:
            print(f'[WARN] Skipping empty crop for {panel["panel_id"]}')
            continue
        if save_crops:
            crop_path = crop_dir / f'{image_path.stem}_{panel["panel_id"]}.png'
            cv2.imwrite(str(crop_path), crop)
        panel_input = load_and_preprocess_panel(crop)
        probs = eff_model.predict(panel_input, verbose=0)[0]
        ...
        panel['all_probs'] = {CLASS_NAMES[i]: float(probs[i]) for i in range(len(CLASS_NAMES))}
        detections.append(panel)

        print(f"  {panel['panel_id']}: {pred_class.upper()} ({confidence*100:.1f}%) | xywh={panel['xywh']}")

    output_image_path = output_dir / f'{image_path.stem}_faults.png'
    annotate_image(image, detections, output_image_path)
    return detections
...
def main():
    ...
    if args.save_crops:
        print(f'[INFO] Saving cropped panels to: {crop_dir or output_dir / "crops"}')

    all_results = []
    for image_path in image_paths:
        detections = process_top_view_image(
        ...
        all_results.append(dict(image=str(image_path), detections=detections))
